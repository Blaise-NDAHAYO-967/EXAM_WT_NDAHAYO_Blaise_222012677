...
<section>
    ...
    <form method="post" action="appointments.php">
        ...
        <label for="duration_minutes">duration_minutes:</label>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['insert'])) {
        $stmt = $connection->prepare("INSERT INTO appointments (appointment_id, customer_id, gardener_id,appointment_date,duration_minutes,created_at,updated_at) VALUES (?, ?, ?,?,?,?,?)");
        $stmt->bind_param("issssss", $a_id, $c_id, $g_id,$a_date,$d_m,$c_at,$u_at);
        $a_id = $_POST['appointment_id'];
        $c_id = $_POST['customer_id'];
        $g_id = $_POST['gardener_id'];
         $a_date = $_POST['appointment_date'];
        $d_m = $_POST['duration_minutes'];
    ...
    ?>
    <!-- Data Display Table -->
    <center><h2>Table of appointments</h2></center>
    <table>
        <tr>
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                echo "<tr>
                    ...
                    <td><a href='delete_appointments.php?appointment_id={$row["appointment_id"]}'>Delete</a></td> 
                    <td><a href='update_appointments.php?appointment_id={$row["appointment_id"]}'>Update</a></td> 
                </tr>";
            }
        } else {
