...
<section>
    ...
    <form method="post" action="plants.php">
        ...
        <input type="text" id="scientific_name" name="scientific_name" required><br><br>
         <label for="plant_type">plant_type:</label>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['insert'])) {
        $stmt = $connection->prepare("INSERT INTO plants (plant_id, name, scientific_name,plant_type,description) VALUES (?, ?, ?,?,?)");
        $stmt->bind_param("issss", $p_id, $n, $s_name,$p_type,$d);
        $p_id = $_POST['plant_id'];
        $n = $_POST['name'];
    ...
    ?>
    <!-- Data Display Table -->
    <center><h2>Table of plants</h2></center>
    <table>
        <tr>
            <th>plant_id</th>
            <th>name</th>
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                echo "<tr>
                    ...
                    <td><a href='delete_plants.php?plant_id={$row["plant_id"]}'>Delete</a></td> 
                    <td><a href='update_plants.php?plant_id={$row["plant_id"]}'>Update</a></td> 
                </tr>";
            }
        } else {
