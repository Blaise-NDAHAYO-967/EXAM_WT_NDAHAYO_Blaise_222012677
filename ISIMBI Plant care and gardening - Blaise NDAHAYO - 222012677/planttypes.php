...
<section>
    <h1>planttypes Form</h1>
    <form method="post" action="planttypes.php">
        <!-- Form Fields -->
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['insert'])) {
        $stmt = $connection->prepare("INSERT INTO planttypes (type_id, name, description) VALUES (?, ?, ?)");
        $stmt->bind_param("iss", $t_id, $n,$d);
       
 $t_id = $_POST['type_id'];         $n = $_POST['name'];
        $d = $_POST['description'];
    ...
    ?>
    <!-- Data Display Table -->
    <center><h2>Table of planttypes</h2></center>
    <table>
        <tr>
            <th>type_id</th>
            <th>name</th>
            ...
            while ($row = $result->fetch_assoc()) {
                echo "<tr>
                   ...
                   <td>{$row["name"]}</td>
                    ...
                    <td><a href='delete_planttypes.php?type_id={$row["type_id"]}'>Delete</a></td> 
                    <td><a href='update_planttypes.php?type_id={$row["type_id"]}'>Update</a></td> 
                </tr>";
            }
        } else {
